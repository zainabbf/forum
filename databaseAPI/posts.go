...
func AddComment(database *sql.DB, username string, postId int, content string, createdAt time.Time) {
	createdAtString := createdAt.Format("2006-01-02 15:04:05")
	statement, _ := database.Prepare("INSERT INTO comments (username, post_id, content, created_at, upvotes, downvotes) VALUES (?, ?, ?, ?, ?, ?)")
	statement.Exec(username, postId, content, createdAtString, 0, 0)
}
